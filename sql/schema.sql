...
BEGIN
    ...
    text_results AS (
        SELECT 
            ...
            c.document_id,
            c.content,
                    ts_rank_cd(to_tsvector('greek', c.content), plainto_tsquery('greek', query_text))::double precision AS text_sim,
            c.metadata,
            d.title AS doc_title,
            d.source AS doc_source
        FROM chunks c
        JOIN documents d ON c.document_id = d.id
        WHERE to_tsvector('greek', c.content) @@ plainto_tsquery('greek', query_text)
    )
    SELECT 
