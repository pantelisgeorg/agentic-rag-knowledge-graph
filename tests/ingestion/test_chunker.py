...
Third paragraph to test chunking."""
        ...
        chunks = await chunker.chunk_document(content, "Multi Para", "multi.md")
    ...
    async def test_chunk_overlap(self):
        ...
        content = "A" * 25 + "\n\n" + "B" * 25 + "\n\n" + "C" * 25 + "\n\n" + "D" * 25
        chunks = await chunker.chunk_document(content, "Overlap Test", "overlap.md")
    ...
    @pytest.mark.asyncio
    async def test_oversized_single_paragraph(self):
        """Test that a single oversized paragraph with no blank lines is split."""
        config = ChunkingConfig(chunk_size=100, chunk_overlap=20)
        chunker = SimpleChunker(config)
        # One long line with no paragraph breaks
        content = "Αυτό είναι μια πρόταση. " * 50
        chunks = await chunker.chunk_document(content, "Long Doc", "long.md")
        assert len(chunks) > 1
        for chunk in chunks:
            assert len(chunk.content) > 0
            assert len(chunk.content) <= config.chunk_size + 50
