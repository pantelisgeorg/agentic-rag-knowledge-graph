...
# Embedding dimension (nomic-embed-text-v2-moe = 768)
...
def mock_embedding_client():
    ...
    with patch('agent.providers.get_embedding_client') as mock_get_client:
        ...
        mock_embedding_response.data = [Mock(embedding=[0.1] * 768)]
...
def mock_graphiti_client():
    """Mock graph client for testing (custom Neo4j client, no Graphiti)."""
    with patch('agent.graph_utils.GraphClient') as mock_client_class:
        mock_client = AsyncMock()
        mock_client_class.return_value = mock_client
        
        # Mock search results (list of fact dicts)
        mock_client.search.return_value = [
            {
                ...
                "valid_at": None,
                "invalid_at": None,
                "source_node_uuid": "TestEntity"
            }
        ]
        
        # Mock entity relationships
        mock_client.get_related_entities.return_value = {
            "central_entity": "Σωκράτης",
            "related_entities": ["Πλάτων", "Αριστοτέλης"],
            "related_facts": [{"fact": "Σωκράτης taught Πλάτων", "uuid": "f1"}],
...
def mock_graph_search_results():
    ...
    return [
        GraphSearchResult(
            fact="Σωκράτης ήταν δάσκαλος του Πλάτωνα",
            uuid="fact-1",
            valid_at=None,
            invalid_at=None,
            source_node_uuid="Σωκράτης"
        ),
        GraphSearchResult(
            fact="Ο Αριστοτέλης ήταν μαθητής του Πλάτωνα",
            uuid="fact-2",
            valid_at=None,
            invalid_at=None,
            source_node_uuid="Αριστοτέλης"
        )
    ]
