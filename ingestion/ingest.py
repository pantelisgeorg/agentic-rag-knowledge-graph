...
class DocumentIngestionPipeline:
    ...
    async def _ingest_single_document(self, file_path: str) -> IngestionResult:
        ...
        # Entity extraction now happens during graph building (single-pass),
        # so no separate extraction step is needed here.
        entities_extracted = 0
        if self.config.extract_entities and self.config.skip_graph_building:
            logger.info("Graph building skipped; entities will not be extracted.")
        ...
        if not self.config.skip_graph_building:
            try:
                ...
                entities_extracted = graph_result.get("entities_extracted", entities_extracted)
