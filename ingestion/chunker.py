...
class SimpleChunker:
    ...
    async def chunk_document(
        ...
        # Further split oversized paragraphs at sentence boundaries so large
        # single-line documents are embedded fully instead of truncated.
        # Reason: the embedding model truncates input beyond ~2048 chars.
        paragraphs = []
        for paragraph in raw_paragraphs:
            ...
            if len(paragraph) > self.config.chunk_size:
                paragraphs.extend(self._split_long_text(paragraph))
            else:
    ...
    def _split_long_text(self, text: str) -> List[str]:
        ...
        Args:
            text: Text longer than chunk_size.

        Returns:
            List of sub-chunks each roughly chunk_size characters.
        """
        sub_chunks = []
        start = 0
        while start < len(text):
            ...
            for i in range(end, max(start + self.config.min_chunk_size, end - 200), -1):
                if text[i] in '.!?\n':
                    chunk_end = i + 1
                    break
            sub_chunks.append(text[start:chunk_end])
            start = chunk_end - self.config.chunk_overlap
        return sub_chunks
